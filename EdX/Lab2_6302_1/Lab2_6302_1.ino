...
//Rest of Setup:
bool first_time;
String config_message_lab_1_6302d1 = "&A~Desired~5&C&S~Direct~O~0~20~0.01&S~Desired~A~-90~90~1&S~Ku~U~0~10~0.01&S~K1~B~0~10~0.01&S~K2~C~-0~10~0.01&S~K3~D~-0~10~0.01&T~ThetaA~F4~-100~100&T~OmegaA~F4~-1000~1000&T~motorBEMF~F4~0~5&T~ErrThetaA~F4~-100~100&T~MCmd~F4~0~5&D~100&H~4&";
...
void loop() {  // Main code, runs repeatedly 
  ...
  float omegaArm = (angleV - pastAngleV[past_size-1])*scaleD;

  // calculate the motor command signal here:
  // Consider subtracting a nominal value from motor_bemf...
  float motorCmd = directV + Ku*desiredV - K1*angleV - K2*omegaArm - K3*motor_bemf;
  ...
  if (loopCounter == numSkip) {  // Lines below are for debugging.
    packStatus(buf, angleV*rad2deg, omegaArm*rad2deg, motor_bemf, errorV*rad2deg, motorCmdLim, float(headroom));
    Serial.write(buf,26);
    loopCounter = 0;
    headroom = deltaT;
  } else loopCounter += 1;
